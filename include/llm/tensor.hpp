...
struct Tensor {
  ...
  explicit Tensor(std::vector<int> shape_)
      : shape(std::move(shape_)),
        data(static_cast<size_t>(numel(shape)), 0.0f) {}
  ...
  float &at(int r, int c) {
    assert(shape.size() == 2 && r >= 0 && r < shape[0] && c >= 0 &&
           c < shape[1]);
  ...
  float *row_ptr(int r) {
    return data.data() + static_cast<size_t>(r) * shape[1];
  }
  ...
  static int numel(const std::vector<int> &s) {
    return std::accumulate(s.begin(), s.end(), 1,
