...
TEST(matmul_identity_is_noop) {
  ...
  for (int i = 0; i < a.size(); ++i)
    ASSERT_NEAR(c.data[i], a.data[i], 1e-6);
}
...
TEST(layernorm_gives_zero_mean_unit_var) {
  Tensor x({2, 4}, {1, 2, 3, 4, 10, 10, 10, 14});
  Tensor g({4}, {1, 1, 1, 1});
  Tensor b({4}, {0, 0, 0, 0});
...
TEST(softmax_stable_for_large_inputs) {
  std::vector<float> v = {1000.0f,
                          1000.0f}; // would overflow without the max-shift
...
TEST(attention_is_causal) {
  ...
  for (int t = 0; t < T; ++t) {
    ...
    qkv.at(t, 2) = 0;
    qkv.at(t, 3) = 0; // k
    qkv.at(t, 4) = (float)(t + 1);
    qkv.at(t, 5) = 0; // v = [t+1, 0]
  }
